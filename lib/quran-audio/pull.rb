...
class Pull
  ...
  def store(res, filename, interrupted:)
    case res
    when Net::HTTPOK
      ...
      throw(:interrupt, true) if interrupted
    else
      puts "error #{res.body}"
    end
  end
